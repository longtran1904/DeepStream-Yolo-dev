...
# Add tools directory to path for GpuMemAccountant
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../../nightly-build/tools"))
from process_gpu_memory_tracker import GpuMemAccountant
...
def run_pipeline(video_path: str, batch_size: int, logger: logging.Logger, process_id: int, logs_dir: str, run_configs_dir: str, gpu_mem_dir: str, gpu_index: int = 0):
    """
    Run deepstream-app for one video with GPU memory profiling.
    - No /tmp usage
    - cwd=BASE_DIR so relative paths in configs work (your requirement)
    - deepstream stdout/stderr redirected to log files only
    - GPU memory tracked via GpuMemAccountant
    """
    ...
    # GPU memory tracking output file
    gpu_mem_out = os.path.join(gpu_mem_dir, f"gpu_deepstream_p{process_id}_{video_basename}.jsonl")
    ...
    logger.info(f"GPU memory tracking: {gpu_mem_out}")
    logger.info(f"cwd (for relative paths): {BASE_DIR}")
    logger.info(f"Command: {' '.join(cmd)}")

    # Initialize GPU memory accountant for this process
    acct = GpuMemAccountant(
        gpu_index=gpu_index,
        out_path=gpu_mem_out,
        run_tag="deepstream_multi_pipeline",
        worker_tag=f"process_{process_id}_{video_basename}",
        sync=True,
        track_total_gpu=True,  # Track total GPU memory to capture deepstream-app subprocess
    )
    ...
    with open(ds_log_path, "ab") as ds_log:
        ...
        try:
            with acct.section("deepstream_pipeline", video=video_name, batch_size=batch_size):
...
def worker(videos, batch_size, shared_index, lock, log_dir, run_configs_dir, gpu_mem_dir, process_id, gpu_index):
    process_name = f"process_{process_id}"
    logger = setup_logging(log_dir, process_name)
    logger.info(f"Worker process {process_id} started on GPU {gpu_index}")
    ...
    while True:
        ...
        video = videos[idx]
        run_pipeline(video, batch_size, logger, process_id, log_dir, run_configs_dir, gpu_mem_dir, gpu_index)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run DeepStream YOLO pipeline on multiple videos in parallel with GPU memory profiling (no /tmp, file-only logs)"
    )
    ...
    parser.add_argument("--gpu-indices", type=str, default="0", help="Comma-separated GPU indices for each process (e.g., '0,1,0,1' for 4 processes)")
    ...
    gpu_indices_str = args.gpu_indices
    
    # Parse GPU indices
    gpu_indices = [int(g.strip()) for g in gpu_indices_str.split(",")]
    if len(gpu_indices) < num_processes:
        # Repeat GPU indices if not enough specified
        gpu_indices = (gpu_indices * ((num_processes // len(gpu_indices)) + 1))[:num_processes]
    else:
        gpu_indices = gpu_indices[:num_processes]
    
    # Logs + per-video generated configs (repo-local; no /tmp usage)
    global LOGS_DIR, RUN_CONFIGS_DIR, GPU_MEM_DIR
    ...
    # Save GPU memory logs in the same directory as other results
    GPU_MEM_DIR = os.path.join(os.path.dirname(LOGS_DIR) if inference_log else BASE_DIR, "gpu_mem_logs")
    
    os.makedirs(LOGS_DIR, exist_ok=True)
    os.makedirs(RUN_CONFIGS_DIR, exist_ok=True)
    os.makedirs(GPU_MEM_DIR, exist_ok=True)
    ...
    main_logger.info(f"GPU_MEM_DIR={GPU_MEM_DIR}")
    main_logger.info(f"GPU indices: {gpu_indices}")
    ...
    for i in range(num_processes):
        p = multiprocessing.Process(
            target=worker, args=(videos, batch_size, shared_index, lock, LOGS_DIR, RUN_CONFIGS_DIR, GPU_MEM_DIR, i, gpu_indices[i])
        )
        p.start()
        processes.append(p)
        main_logger.info(f"Started worker process {i} (pid={p.pid}, gpu={gpu_indices[i]})")
